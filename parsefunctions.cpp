 ...
 */
std::string get_file_path(std::string line, struct SERVER_PARAMS* server, bool raw) {
    ...
    std::vector<std::string> request_split = split(path, '?');

    // clean get parameters from path
    path = request_split[0];

    request_split = subarr(request_split, 1, request_split.size());

    std::string param_str = join(request_split, "?");

    for(std::string p : split(param_str, '&')) {
        std::vector<std::string> param = split(p, '=');

        std::string name = param[0];
        param = subarr(param, param.size() - 1, param.size());
        std::string content = join(param, "=");
...
std::string get_mime_type(std::string file) {
    std::vector<std::string> broken = split(file, '.');
